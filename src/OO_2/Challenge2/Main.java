...
public class Main {
    public static void main(String[] args) {
        ...
        for (Studio stud : studioDatabase) {
            //1
            if (stud.getFilme().length > 2)
                System.out.println(stud.getNume());
            //2
            for (Film filme : stud.getFilme())
                for (Actor actori : filme.getActori())
                    if (actori.getNume().equals("actor cu 2 oscaruri"))
                        System.out.println(stud.getNume());
        //3
        }
        for (Studio stud : studioDatabase) {
            for (Film filme : stud.getFilme())
                for (Actor actori : filme.getActori())
                    if (actori.getVarsta() > 50)
                        System.out.println(filme.getNume());
        }
    }
}
